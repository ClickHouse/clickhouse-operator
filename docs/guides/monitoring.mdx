...
<Note>
This guide is about the **operator process itself** (the controller manager). For ClickHouse server metrics (queries, parts, replication lag), enable the [Prometheus endpoint in ClickHouse](/operations/server-configuration-parameters/settings#prometheus) via `extraConfig` and scrape it separately.
</Note>
...
|---|---|---|---|
| Metrics | `8080` (helm) / `:0` disabled (raw flag default) | `/metrics` | Prometheus exposition format |
...
- `Service/<release>-controller-manager-metrics-service` — exposes port `8080` (HTTPS when `metrics.secure: true`).
- `ServiceMonitor/<release>-controller-manager-metrics-monitor` — when `prometheus.enable: true`.
- `ClusterRole/clickhouse-operator-metrics-reader` — non-resource URL `/metrics` with `get` verb.
...
<Warning>
If you see `401 Unauthorized` or `403 Forbidden` from the metrics endpoint, the scraper is either presenting no bearer token (HTTP scraping a HTTPS endpoint) or its ServiceAccount lacks the binding above. Disabling security by setting `metrics.secure: false` is **not recommended** in shared clusters because anyone with network reachability to the pod could scrape the endpoint.
</Warning>
...
spec:
  ...
  endpoints:
    - path: /metrics
      ...
      tlsConfig:
        serverName: <release>-controller-manager-metrics-service.<namespace>.svc
        ca:
          secret:
...
kubectl -n $NS run curl-metrics --rm -it --restart=Never \
  --image=curlimages/curl:8.10.1 -- sh -c '
    ...
    curl -sk -H "Authorization: Bearer $TOKEN" \
      https://<release>-controller-manager-metrics-service.'$NS'.svc:8080/metrics \
      | head -20
  '
```
