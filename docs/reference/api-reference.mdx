...
| `keeperClusterRef` | [KeeperClusterReference](#keeperclusterreference) | Reference to the KeeperCluster that is used for ClickHouse coordination.<br />When namespace is omitted, the ClickHouseCluster namespace is used.<br />Mutually exclusive with `externalKeeper`, exactly one of them must be set. | false |  |
| `externalKeeper` | [ExternalKeeperSpec](#externalkeeperspec) | Keeper ensemble that is not managed by this operator, used for ClickHouse coordination.<br />The operator does not own the lifecycle of these servers and only points ClickHouse at them.<br />Mutually exclusive with `keeperClusterRef`, exactly one of them must be set. | false |  |
...
| `systemLogsTTLDays` | integer | Retention in days for the system log tables (`query_log`, `part_log`, `text_log`, `metric_log`,<br />`asynchronous_metric_log`), applied as a table TTL on the next server restart. Defaults to 30 days,<br />0 disables the TTL. | false |  |
...
## ExternalKeeperNode {#externalkeepernode}

ExternalKeeperNode addresses a single server of an external Keeper ensemble.

| Field | Type | Description | Required | Default |
|-------|------|-------------|----------|---------|
| `host` | string | Host name or IP address of the Keeper server. | true |  |
| `port` | integer | Port the Keeper server accepts client connections on. | false | 9181 |

Appears in:
- [ExternalKeeperSpec](#externalkeeperspec)
...
| Field | Type | Description | Required | Default |
|-------|------|-------------|----------|---------|
| `nodes` | [ExternalKeeperNode](#externalkeepernode) array | Servers of the Keeper ensemble ClickHouse connects to for coordination. | true |  |
| `tls` | [ExternalKeeperTLSPolicy](#externalkeepertlspolicy) | TLS controls whether ClickHouse connects to the ensemble over TLS.<br />The ports in nodes must match the chosen policy. | false | Disabled |

Appears in:
- [ClickHouseClusterSpec](#clickhouseclusterspec)

## ExternalKeeperTLSPolicy {#externalkeepertlspolicy}

ExternalKeeperTLSPolicy controls how ClickHouse connects to an external Keeper ensemble.

| Field | Description |
|-------|-------------|
| `Enabled` | ExternalKeeperTLSEnabled makes ClickHouse connect to the ensemble over TLS.<br /> |
| `Disabled` | ExternalKeeperTLSDisabled is the default policy: connections are made in plain text.<br /> |
