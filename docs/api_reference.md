...
| `required` | `bool` | No | Require TLS for all connections (no plaintext). |
...
| `ReconcileSucceeded`      | Last reconciliation loop successfullness                                                           |
...
| `ConfigurationInSync`     | Represents Configuration rollout state                                                             |
| `Ready`                   | ClickHouseCluster is ready to serve client requests. At least one reaplica in every shard is Ready |
